...
class ChangePassowrdFragment : BaseFragment() {
    ...
    private var isShowCurrentPass = false
    private var CurrentPassword=false
    ...
    override fun onCreateView(
        ...
        mView.iv_show_current_pass.setOnClickListener {
            if (isShowCurrentPass)
            {
                isShowCurrentPass = false
                mView.iv_show_current_pass.setImageResource(R.drawable.ic_baseline_visibility_off_24)
                mView.et_current_pass.transformationMethod = PasswordTransformationMethod()
            }
            else
            {
                isShowCurrentPass = true
                mView.iv_show_current_pass.setImageResource(R.drawable.ic_baseline_visibility_24)
                mView.et_current_pass.transformationMethod = null
            }

        }


        if(arguments?.getString("enable").toString().equals("yes")){
            mView.current_layout.visibility=View.VISIBLE
    ...
    private fun validateFields() {
        ...
        if(CurrentPassword) {
            if (!mView.et_current_pass.text.toString().isValidPassword()) {
                mView.et_current_pass.setError(getString(R.string.password_must_be))
                return
            }
        }
